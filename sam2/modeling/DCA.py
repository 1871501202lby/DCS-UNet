...
#         self.pooling = pooling(output_size=patch)
#         self.patch = patch
#     def forward(self, x):
#         # x = self.projection(x)
...
class DCA(nn.Module):
    ...
    def forward(self, raw):
        ...
        for idx, tensor in enumerate(raw):
            tensor_proj, (H, W) = self.patch_avg[idx](tensor)
            x.append(self.avg_map[idx](tensor_proj))
        ...
        x = [self.upconvs[i](xi) for i, xi in enumerate(x)]

        # 动态上采样并与原始张量相加
        x_out = []
        for xi, orig_size, raw_tensor in zip(x, orig_sizes, raw):
            scale = (orig_size[0] / xi.size(2), orig_size[1] / xi.size(3))
            xi_up = nn.functional.interpolate(xi, scale_factor=scale, mode='bilinear', align_corners=True)
            sum_tensor = xi_up + raw_tensor
            x_out.append(sum_tensor)
...
if __name__ == '__main__':
    ...
    y = torch.randn(1, 64, 44, 44)
    z = torch.randn(1, 64, 22, 22)
    v = torch.randn(1, 64, 11, 11)
    model = DCA([64,64,64,64])
